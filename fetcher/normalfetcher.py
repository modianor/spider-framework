...
class NormalFetcher(Fetcher):
    ...
    def parseList(self, context, html):
        ...
        listParams = context.get('listParams', [])
        if 'list' in config:
            ...
            for i in range(dataLen):
                ...
                infos.append([info, '', '', f'{policyId}|Detail'])
            self.logger.info(f'ListParse解析当前页【{context["url"]}】得到{len(infos)}个listParams')
            context['listParams'] = listParams + infos

    def turnPage(self, context):
        urlList = []
        preHtml = ''
        config = context.get('config', {})
            ...
            if "autoPage" in pageParseConfig and pageParseConfig["autoPage"]:
                ...
                for i in range(pageBegin, 10000000000):
                    ...
                    if threshold > sameDiff:
                        ...
                        context['url'] = preUrl
                        # 这里做解析保存处理
                        self.parseList(context, preHtml)
                        # 继续翻页
                        preHtml = curHtml
                        preUrl = curUrl
                    # 超过指定结束页码
                ...
                if "pageNums" in pageParseConfig:
                    ...
                    values = self.parseFieldByRule(preHtml, 'regex', pageNumsConfig)
                    ...
                    for curUrl in urlList:
                        ...
                        curHtml = response.text
                        context['url'] = curUrl
                        self.parseList(context, curHtml)
